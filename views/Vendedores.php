...
?>
    ...
    <div id="response-msg-2" class="hide">
        <p class="text-center">
            <strong>Salario Base: R$</strong>
            <span id="f1"></span>
        </p>
        <p class="text-center">
            <strong>Comissão calculada: R$</strong>
            <span id="f2"></span>
        </p>
        <p class="text-center">
        ...
        <btn id="close" class="btn-success" onclick="closemsg2()">Ok!</btn>
    </div>
                ...
                <table class="table mt-3 mb-3">
                    ...
                    <?php foreach ($vendedores as $vendedor) : ?>
                        <tr>
                            ...
                            <td>
                                <i id="edit-<?php echo $vendedor['id']; ?>"
                                   onclick="editVendedor(<?php echo $vendedor['id']; ?>)" class="fas fa-edit"
                                   data-toggle="tooltip" title="Editar"></i>
                                <i id="delete-<?php echo $vendedor['id']; ?>"
                                   onclick="deleteVendedor(<?php echo $vendedor['id']; ?>)" class="fas fa-trash-alt"
                                   data-toggle="tooltip" title="Deletar"></i>
                                <i id="delete-<?php echo $vendedor['id']; ?>"
                                   onclick="calcSalarioVendedor(<?php echo $vendedor['id']; ?>)"
                                   class="fas fa-comment-dollar" data-toggle="tooltip" title="Calcular salario"></i>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                    </tbody>
    ...
    <script>
        ...
        function calcSalarioVendedor(id) {
            var endpoint = "<?php echo SITE_URL; ?>/vendedor/calcComissao/" + id
            request = $.ajax({
                url: endpoint,
                type: "post",
            });
            ...
            request.done(function (response, textStatus, jqXHR) {
                ...
                if (response) {
                    ...
                    let salarioBase = parseFloat($("#salario-" + id).text().replace('.', '').replace(',', '.'));
                    let comissao = parseFloat(response);
                    let total = salarioBase + comissao;
                    console.log("salario base: " + salarioBase)
                    console.log("comissao: " + comissao)
                    console.log("total: " + total)
                    showResponse2(salarioBase, comissao, total);
                } else {
                    showResponse("Erro ao gerar Salario!", "F");
                }
            });
        ...
        function showResponse2(f1, f2, f3) {
            $("#f1").text(f1.toFixed(2))
            $("#f2").text(f2.toFixed(2))
            $("#f3").text(f3.toFixed(2))
            $("#response-msg-2").removeClass("hide");
            $("#response-msg-2").addClass("show");

        }

        function closemsg2() {
            $("#response-msg-2").removeClass("show");
            $("#response-msg-2").addClass("hide");
        }
    </script>
